...
contract LiquidToken {
    ...
    uint constant mandatoryLockinPeriods = 2; // example: funds may stay at least 1 complete period (2 period closings)
    uint periodsMovingAverage = 100 ; // Use moving average of control metric.
    // Financial Constants
    uint baseScale = 10^18 ;
    uint constant tokenFundsPerBaseCoin = 1 ; // 1 wei of collateral equivalent in TRDs base subdivision   //100000000000000000; // example: 0.1 ETH per 1 TRD for funding
    // Estimate max cost in gas between fund(), refund() and transfer(), using JavaScript
    int constant minCostInGasOfTransaction = 21000 ;
    uint collapseTolerance = baseScale*9/10 ;//ufixed0x128(0.9) ;
    // Options constants
    uint beginOptExecutionDate = 180 ; // team can execute options starting these days after Contract begin.
    uint endOptExecutionDate = 365 ; // team can execute options starting these days after Contract begin.
    ...
    function Toroid() {
    	     creationDate = now / secondsPerPeriod; // alias of block.timestamp, translated to days since epoch
        lastUpdateGlobal = 0 ; //creationDate ; //- 1 ;
        periodRebasements[lastUpdateGlobal] = baseScale; // equivalent to 1.0, so no rebasement initially.
    }
	...
	if (lastUpdate[wallet] == 0) {
	       rebasements[wallet] = baseScale ; // equivalent to 1.0 in our fixed point scale.
	       			   }
				       else {
																								      		      ...
																								      		      if (rateControl < 0) {
																										      	     	      absRateControl = rateControl * -1 ;
																																			 ...
																																			 // Reflective Currency manipulation limit.
																																			    // Worst case scenario, the attacker jumps transactions from 1 to a big number.
																																			    
																																			     int deltaControl = (int(periodTransactions[currentPeriod-1])-int(periodTransactions[currentPeriod-2])) ; // * maxPriceInGasOfTransaction ;
																																			     	 // abs(a-b)
																																				    if (deltaControl < 0)
																																				       		     deltaControl *= -1 ;
																																																					        ...
																																																					        finalRate = rateProtection ;
																																																							    finalRate *= signRateControl ;
																																																								       ...
																																																								       // do (R + 1.0 ) * earlyAdoptersRebasement
																																																								       	  return uint(finalRate + int(baseScale)) * earlyAdoptersPremium ; // rate + 1.0   
    }

    function checkBurstReturnCollateral() private {
    	     // Probably not necessary.
    	     // If #TRD in circulation are few than rebasement goes below 1.0 (meaning 1 ETH is less worth than 1 TRD)
    	     // meaning there are very few TRD, small supply, and TRD is appreciating too much.
    ...
    function fund() payable returns (bool success) {
    	     //updateBalance(msg.sender) ; // to avoid funding of past rebasements in case of price falling.
	     				 // Check team options
					    uint fundMultiplier = 1 ;
					    	 uint ageInPeriods = 0 ;
						      ageInPeriods = now / secondsPerPeriod - creationDate  ;
									...
									/*
										int teamIndex = isTeamWallet(msg.sender) ;
										    if (teamIndex >= 0 && ageInPeriods > beginOptExecutionDate && ageInPeriods < endOptExecutionDate){ // belong to team
										       		  fundMultiplier = teamOptionsMultiplier ; 
												  		   // we use return because we have nothing to revert and also throw is not working in our test deployment.
														      	 if (optionsAvailable[uint(teamIndex)]*baseScale < msg.value*fundMultiplier) return false;
															    						 optionsAvailable[uint(teamIndex)] -= msg.value*fundMultiplier ;
																					 				   }
																									    */
																									     fundBalances[msg.sender] += msg.value*fundMultiplier ;
																									     			      // one-way peg, for one period there is no incentive to dump the new coins. 
																													       ...
																													       // but there is not minimum a-priori.
																													       	  balances[msg.sender] += msg.value * tokenFundsPerBaseCoin ;
																														  		       lastFundPeriod[msg.sender] =  now / secondsPerPeriod - creationDate ; 
																																       				  ...
																																       				  //periodTransactions[currentPeriod] =  periodTransactions[currentPeriod] + 1 ;
    ...
    // will go through.
    // TODO: use Alarm Clock from pipinmerriam (https://github.com/pipermerriam/ethereum-alarm-clock)
